...
describe('Block', () => {
  ...
  describe('.create(properties) (static)', () => {
    ...
    it('should return `properties` if it\'s an instance of `Block`', () => {
      let block1 = Block.create({ type: 'fsm-node' });
      let block2 = Block.create(block1);

      assert.strictEqual(block1, block2);
      assert.deepEqual(block1.toJS(), block2.toJS());
    });
  ...
  describe('.createList(elements) (static)', () => {
    ...
    it('should return `immutable.List` of `Block`s if `elements` not an instance of `immutable.List`', () => {
      let elements = [
        { key: 'block-1', type: 'fsm-node' },
        { key: 'block-2', type: 'fsm-node' },
        { key: 'block-3', type: 'fsm-node' }
